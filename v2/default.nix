...
in
{
  options.services.gallipedal.v2 = {
  ...
  config =
    let
      internalProxyRules =
        if builtins.hasAttr "internalRules" config.services.gallipedal.v2.proxyConf
        then config.services.gallipedal.v2.proxyConf.internalRules
        else "";
      reverseProxyNetwork =
        if builtins.hasAttr "network" config.services.gallipedal.v2.proxyConf
        then config.services.gallipedal.v2.proxyConf.network
        else "";
      proxyTLSResolver =
        if builtins.hasAttr "tlsResolver" config.services.gallipedal.v2.proxyConf
        then config.services.gallipedal.v2.proxyConf.tlsResolver
        else "";

      serviceDefinitions = lib.attrsets.getAttrs
        config.services.gallipedal.v2.services
        servicesLibrary;
    ...
    in
    lib.mkIf config.services.gallipedal.v2.enable {
