...
rec {
  ...
  foldListToOptions = func: defnList:
    (lib.lists.foldl (acc: defn:
      acc // (func defn)
    ) { } defnList);

  mkPortOptions = portDefs:
    foldListToOptions (portDef: {
      ...
      "${volumeDef.containerPath}" = lib.mkOption {
        type = mkSubmodule {
          ...
          extraPerms = lib.mkOption {
            ...
            default = if builtins.hasAttr "extraPerms" volumeDef
  ...
  mkContainerOptions = conDef: {
    ...
    extraLabels = lib.mkOption {
      ...
      default = if builtins.hasAttr "extraLabels" conDef
    ...
    extraOptions = lib.mkOption {
      ...
      default = if builtins.hasAttr "extraOptions" conDef
    ...
    cmd = lib.mkOption {
      type = lib.types.listOf lib.types.str;
      default = if builtins.hasAttr "cmd" conDef
        then conDef.cmd
        else [ ];
    };
    ...
    networks = lib.mkOption {
      type = lib.types.attrs;
      default = if builtins.hasAttr "networks" conDef
    ...
    dependsOn = lib.mkOption {
      type = lib.types.listOf lib.types.str;
      default = if builtins.hasAttr "dependsOn" conDef
    ...
    proxies = lib.mkOption {
      type = lib.types.listOf lib.types.attrs;
      default = if builtins.hasAttr "proxies" conDef
        then conDef.proxies
        else [ ]; 
    };
